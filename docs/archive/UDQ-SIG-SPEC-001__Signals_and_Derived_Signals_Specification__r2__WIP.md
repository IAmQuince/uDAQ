...
depends_on:
  ...
  - "UDQ-GOV-STD-002"
supersedes:
  - "UDQ-SIG-SPEC-001__Signals_and_Derived_Signals_Specification__r1__WIP.md"
revision_history:
...
This active revision is now read together with `UDQ-LIFECYCLE-SPEC-001` and `UDQ-PERF-SPEC-001` so signal and variable propagation semantics remain aligned with the bounded performance doctrine. [SEC:UDQ-SIG-SPEC-001::0]

## Revision History [SEC:UDQ-SIG-SPEC-001::0.1]
- r2: Subsystem reconciliation pass: clarified signal-vs-command semantics, quality-state ownership, and anti-conflation rules.
...
## 2A. Semantic Closure and Anti-Conflation Rule [SEC:UDQ-SIG-SPEC-001::2A]

This specification shall operationalize the glossary-owned meaning of **signal**, **derived signal**, **quality state**, **live**, **historical**, and **simulated** without redefining them. A signal is a runtime data object; it is not itself an output ownership record, authorization record, or command result. Derived signals may inform rules, diagnostics, and UI explanation, but they shall not collapse the distinction between requested state, applied state, and observed state that is owned by the output model.
...
## 2B. Variables relationship note [SEC:UDQ-SIG-SPEC-001::2B]
Variables are now a first-class downstream layer in the active package line. For the current bounded package, this specification continues to own the canonical signal concepts, while `UDQ-LIFECYCLE-SPEC-001` and `UDQ-REQ-ADD-001` own the lifecycle, reconciliation, and requirement-side posture for variables.

Signals remain the canonical measured or directly platform-published values. Variables are the bounded expression/alias/state layer that may depend on signals, constants, or other variables without collapsing signal identity into ad hoc UI-only fields.
