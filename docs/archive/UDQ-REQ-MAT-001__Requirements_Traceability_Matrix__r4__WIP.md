...
revision_history:
  - revision: r3
    date: 2026-03-21
    summary: Granularity refresh to absorb Phase 3 runtime subsystem docs and Phase 4 UI detail docs with clearer requirement-to-spec mapping, plus stable section anchors.
  - revision: r2
    date: 2026-03-21
    summary: Machine-readable normalization pass.
---
# Requirements Traceability Matrix [SEC:UDQ-REQ-MAT-001::0]

## 1. Purpose [SEC:UDQ-REQ-MAT-001::1]

This matrix maps requirement identifiers to governing sources, downstream subsystem specifications, proof expectations, and package-level evidence obligations. Revision r3 tightens coverage for alarms, diagnostics, persistence/restore, device abstraction, export/evidence bundles, authorization, shell/navigation, workspace pages, graphing, rules-editor roundtrip behavior, visual language, and remote UI surfaces.

## 2. Requirement Columns [SEC:UDQ-REQ-MAT-001::2]

Each row includes:
- Requirement ID
- Requirement statement
- Primary source documents
- Downstream specification documents
- Verification / proof expectations
- Package / runtime evidence expectations
- Current status

## 3. Requirement Rows [SEC:UDQ-REQ-MAT-001::3]
...
## 4. Notes [SEC:UDQ-REQ-MAT-001::4]

This matrix is intentionally requirement-oriented rather than implementation-complete. Additional rows may still be added as implementation planning becomes more concrete, but the current revision closes the most obvious granularity gap between foundational doctrine and the Phase 3 / Phase 4 subsystem layers.
