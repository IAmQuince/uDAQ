...
depends_on:
  ...
  - "UDQ-SEC-SPEC-001"
supersedes:
  - "UDQ-OUT-SPEC-001__Outputs_Command_Arbitration_and_Safe_State_Specification__r1__WIP.md"
revision_history:
  - "r4 | 2026-03-27 | Global documentation reconciliation: documented same-owner renewal, stale-expiry handling, target-unavailable rejection, lease revocation on degrade, and bounded safe-state behavior for the first write-side slice."
  - "r3 | 2026-03-27 | Bounded cross-device command sprint: canonical writable tags, ownership lease semantics, degraded-target rejection, and safe-state review events aligned to the unified evidence spine."
  - "r2 | 2026-03-21 | Subsystem reconciliation pass: locked requested/applied/observed semantics, backend arbitration ownership, and safe-state anti-conflation rules."
...
- r4: Global documentation reconciliation: documented same-owner renewal, stale-expiry handling, target-unavailable rejection, lease revocation on degrade, and bounded safe-state behavior for the first write-side slice.
- r3: Bounded cross-device command sprint: canonical writable tags, ownership lease semantics, degraded-target rejection, and safe-state review events aligned to the unified evidence spine.
- r2: Subsystem reconciliation pass: locked requested/applied/observed semantics, backend arbitration ownership, and safe-state anti-conflation rules.
...
## 0A. Current bounded implementation note [SEC:UDQ-OUT-SPEC-001::0A]

The current implementation-entry package now includes a bounded cross-device command lane that proves:
- canonical writable tags remain device-agnostic in the core,
- explicit ownership leases and conflict rejection are first-class outcomes,
- degraded targets are rejected conservatively rather than guessed,
- safe-state requirements are emitted as explicit review events, and
- command intents, adapter outcomes, and arbitration events remain inside the same runtime evidence spine used by read-side acquisition.

This note does **not** broaden the package claim to full output completeness, generalized sequencing, or production-rate actuation.


## 0B. Gap-hardening closeout note [SEC:UDQ-OUT-SPEC-001::0B]

The current bounded write-side slice is now documented as having the following edge behaviors explicitly closed in first form:
- same-owner lease renewal is a visible journaled event rather than an implicit mutation,
- stale leases expire deterministically,
- target-unavailable commands are rejected explicitly,
- degraded targets revoke active leases so stale command ownership is not resurrected on restore,
- near-simultaneous adapter degradation remains bounded and evidence-producing,
- safe-state-required events are visible as first-class command-side truth.

These behaviors remain intentionally bounded. They do not yet imply broad output completeness, sequencing depth, or generalized multi-device actuation maturity.
...
## 2A. Semantic Closure and Anti-Conflation Rule [SEC:UDQ-OUT-SPEC-001::2A]

This specification is the operational owner for output-command semantics beneath the glossary. **Requested state**, **applied state**, and **observed state** shall remain separate records. Arbitration, ownership, authorization, interlocks, permissives, inhibits, and safe-state behavior shall be backend-governed and evidence-producing. Subordinate UI and remote documents may visualize these concepts but shall not redefine them.
...
## 20. Gap-Hardening Notes for the First Cross-Device Command Slice [SEC:UDQ-OUT-SPEC-001::20]

The first bounded cross-device command slice shall explicitly harden the following edges before a broader control-environment binding sprint is opened:
- same-owner renewal of a writable-tag lease shall be explicit and attributable,
- lease revocation on degraded target transition shall be explicit and journaled,
- target reintroduction shall not resurrect stale command intents,
- degraded and near-simultaneous multi-adapter loss shall result in bounded rejection/failure rather than silent success,
- ownership ledger artifacts shall describe lifecycle transitions, not only end-state snapshots.

The bounded acceptance surface for this slice shall continue to use the existing one-command entry and shall remain additive to the read-side cross-device proof lane rather than introducing a separate command-only harness.


## 6A. Current bounded ownership lease rules [SEC:UDQ-OUT-SPEC-001::6A]

For the current bounded cross-device write-side slice, the active implementation shall be interpreted as having these first-form rules:
- one writable tag may have at most one active lease holder at a time,
- same-owner renewal extends or refreshes the active lease and is journaled as `ownership_renewed`,
- non-owner command attempts during an active lease are rejected explicitly,
- stale leases expire deterministically and do not linger as implied ownership,
- target degradation revokes affected active leases rather than leaving ambiguous ownership behind.

## 10A. Current bounded degraded-output doctrine [SEC:UDQ-OUT-SPEC-001::10A]

For the current bounded slice, degraded output behavior shall be interpreted conservatively:
- uncertain commands are rejected rather than guessed,
- target-unavailable outcomes are explicit and journaled,
- reconnect does not silently replay stale intents,
- safe-state-required events are explicit runtime truth,
- degraded and reintroduced device transitions remain reviewable through the same evidence spine used for read-side truth.
