...
class MappingDryRunCommitState(StrEnum):
    ACCEPTED = 'accepted'
    REJECTED = 'rejected'
    STALE = 'stale'
    BACKEND_UNAVAILABLE = 'backend_unavailable'
...
class MappingApplyResult:
    ...
    created_timestamp: EventTime
    execution_mode: str
    state: MappingDryRunCommitState
    authorized: bool
    accepted: bool
    executed_live: bool
    ...
    proposal_summaries: tuple[str, ...] = field(default_factory=tuple)
    audit_event: MappingApplyAuditEvent | None = None

    @property
    def audit_summary(self) -> str:
        if self.accepted:
            return f'dry-run commit accepted for {self.would_change_count} mapping change(s); no live execution occurred'
        return f'dry-run commit rejected: {self.blocked_reason or self.state.value}; no live execution occurred'


@dataclass(frozen=True, slots=True, kw_only=True)
class MappingApplyCoordinator:
    """Controller-owned non-mutating dry-run commit boundary.

    This coordinator intentionally does not mutate authoritative bindings. It
    revalidates a prepared request against the current authoritative readback
    snapshot and returns a structured dry-run result/audit event. Live apply
    modes are rejected explicitly so future live commit work cannot accidentally
    piggyback on the dry-run path.
    """

    allowed_dry_run_modes: frozenset[MappingApplyMode] = frozenset({MappingApplyMode.DRY_RUN_COMMIT})

    def dry_run_commit(
        self,
        *,
        request: MappingApplyRequest,
        current_authoritative_snapshot_timestamp: EventTime | None,
        readback_available: bool,
        created_timestamp: EventTime,
        execution_mode: MappingApplyMode | str = MappingApplyMode.DRY_RUN_COMMIT,
        request_consumed: bool = False,
        result_id: str | None = None,
        audit_event_id: str | None = None,
    ) -> MappingApplyResult:
        mode = _coerce_apply_mode(execution_mode)
        proposal_summaries = tuple(
            f'{proposal.change_kind.value}:{proposal.logical_id}:{proposal.current_endpoint or "<none>"}->{proposal.proposed_endpoint or "<none>"}'
            for proposal in request.proposals
        )
        if mode not in self.allowed_dry_run_modes:
            return self._reject(
                request=request,
                created_timestamp=created_timestamp,
                execution_mode=str(execution_mode),
                state=MappingDryRunCommitState.LIVE_MODE_REJECTED,
                reason='live mapping apply execution modes are not available at the controller dry-run boundary',
                proposal_summaries=proposal_summaries,
                result_id=result_id,
                audit_event_id=audit_event_id,
            )
        if request.mode == MappingApplyMode.EXECUTE_LIVE:
            return self._reject(
                request=request,
                created_timestamp=created_timestamp,
                execution_mode=mode.value,
                state=MappingDryRunCommitState.LIVE_MODE_REJECTED,
                reason='prepared request asks for execute_live; only non-mutating dry-run commit is allowed',
                proposal_summaries=proposal_summaries,
                result_id=result_id,
                audit_event_id=audit_event_id,
            )
        if not readback_available:
            return self._reject(
                ...
                created_timestamp=created_timestamp,
                execution_mode=mode.value,
                state=MappingDryRunCommitState.BACKEND_UNAVAILABLE,
                reason='authoritative readback became unavailable before controller dry-run commit',
                proposal_summaries=proposal_summaries,
                result_id=result_id,
                audit_event_id=audit_event_id,
            )
        if request.preflight_state not in {MappingPreflightState.PASS.value, MappingPreflightState.PASS_WITH_WARNINGS.value}:
            return self._reject(
                request=request,
                created_timestamp=created_timestamp,
                execution_mode=mode.value,
                state=MappingDryRunCommitState.REJECTED,
                reason=f'prepared request preflight state {request.preflight_state!r} is not eligible for dry-run commit',
                proposal_summaries=proposal_summaries,
                result_id=result_id,
                audit_event_id=audit_event_id,
            )
        if request.authoritative_snapshot_timestamp is None or current_authoritative_snapshot_timestamp is None:
            return self._reject(
                request=request,
                created_timestamp=created_timestamp,
                execution_mode=mode.value,
                state=MappingDryRunCommitState.STALE,
                reason='authoritative readback snapshot timestamp is missing at commit boundary',
                proposal_summaries=proposal_summaries,
                result_id=result_id,
                audit_event_id=audit_event_id,
            )
        if request.authoritative_snapshot_timestamp != current_authoritative_snapshot_timestamp:
            return self._reject(
                request=request,
                created_timestamp=created_timestamp,
                execution_mode=mode.value,
                state=MappingDryRunCommitState.STALE,
                reason='prepared request was based on a stale authoritative readback snapshot',
                proposal_summaries=proposal_summaries,
                result_id=result_id,
                audit_event_id=audit_event_id,
            )
        if request_consumed:
            return self._reject(
                ...
                execution_mode=mode.value,
                state=MappingDryRunCommitState.REJECTED,
                reason='prepared request has already been consumed by a prior commit attempt',
                proposal_summaries=proposal_summaries,
                result_id=result_id,
                audit_event_id=audit_event_id,
            )
        audit = MappingApplyAuditEvent(
            event_id=audit_event_id or f'map-apply-audit::{uuid4()}',
            ...
            accepted=True,
            executed_live=False,
            reason='dry-run commit accepted; authoritative state was not mutated',
            operator_summary=f'Dry-run accepted; would change {len(request.proposals)} mapping(s); not executed live.',
        )
        return MappingApplyResult(
            result_id=result_id or f'map-apply-result::{uuid4()}',
            request_id=request.request_id,
            created_timestamp=created_timestamp,
            execution_mode=mode.value,
            state=MappingDryRunCommitState.ACCEPTED,
            authorized=True,
            accepted=True,
            executed_live=False,
            would_change_count=len(request.proposals),
            authoritative_snapshot_timestamp=current_authoritative_snapshot_timestamp,
            warnings=tuple(_extract_warning_lines(request.review_summary)),
            proposal_summaries=proposal_summaries,
            audit_event=audit,
        )
    ...
    def _reject(
        ...
        *,
        ...
        audit = MappingApplyAuditEvent(
            ...
            executed_live=False,
            reason=reason,
            operator_summary=f'Dry-run rejected: {reason}. Not executed live.',
        )
        return MappingApplyResult(
            result_id=result_id or f'map-apply-result::{uuid4()}',
            request_id=request.request_id,
            ...
            authoritative_snapshot_timestamp=request.authoritative_snapshot_timestamp,
...
def _coerce_apply_mode(mode: MappingApplyMode | str) -> MappingApplyMode | str:
    if isinstance(mode, MappingApplyMode):
        return mode
    try:
        return MappingApplyMode(str(mode))
    except ValueError:
        return str(mode)


def _extract_warning_lines(review_summary: str) -> tuple[str, ...]:
    return tuple(line for line in review_summary.splitlines() if 'warning' in line.lower())
