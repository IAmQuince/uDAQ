...
CURRENT_PACKAGE_ID = 'UDQ-PKG-20260330-CONTROLLER-AUTHORIZED-MAPPING-APPLY-DRY-RUN-COMMIT-BOUNDARY-R01'
CURRENT_PACKAGE_SLUG = 'controller-authorized-mapping-apply-dry-run-commit-boundary'
...
class ShellController:
    ...
    def dry_run_commit_mapping_apply_request_for_device(
        self,
        *,
        ...
        created_timestamp: EventTime,
        execution_mode: MappingApplyMode = MappingApplyMode.DRY_RUN_COMMIT,
        request_consumed: bool = False,
        coordinator: MappingApplyCoordinator | None = None,
    ) -> MappingApplyResult:
        inventory = self.authoritative_binding_inventory(device_identity_key=device_identity_key)
        active_coordinator = coordinator or MappingApplyCoordinator()
        return active_coordinator.dry_run_commit(
            request=request,
            current_authoritative_snapshot_timestamp=current_authoritative_snapshot_timestamp,
            readback_available=inventory.readback_available,
            created_timestamp=created_timestamp,
