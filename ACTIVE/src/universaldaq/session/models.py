...
SESSION_AUTHORITY_SCOPE = "review_session_only"
...
class SessionCheckpoint:
    ...
    def authority_scope(self) -> str:
        return SESSION_AUTHORITY_SCOPE

    @property
    def runtime_snapshot_hash(self) -> str:
        return _state_hash({"runtime_snapshot": _json_value(dict(self.runtime_snapshot))})

    @property
    def checkpoint_hash(self) -> str:
        return _state_hash(self._to_dict(include_checkpoint_hash=False))

    def _to_dict(self, *, include_checkpoint_hash: bool) -> dict[str, object]:
    ...
    def __post_init__(self) -> None:
        if self.runtime_snapshot_model_version is None:
            ...
            object.__setattr__(
                self,
                "runtime_snapshot_model_version",
                None if model_version is None else str(model_version),
            )

    def to_dict(self) -> dict[str, object]:
        return self._to_dict(include_checkpoint_hash=True)

    @staticmethod
    def from_dict(payload: Mapping[str, object]) -> SessionCheckpoint:
        runtime_snapshot = _payload_mapping(
            payload.get("runtime_snapshot", {}), field_name="runtime_snapshot"
        )
        warnings = _payload_sequence(payload.get("warnings", ()), field_name="warnings")
        known_limitations = _payload_sequence(
            payload.get("known_limitations", ()), field_name="known_limitations"
        )
        ...
        return SessionCheckpoint(
            ...
            if payload.get("source_package_tag") in {None, ""}
            else str(payload.get("source_package_tag")),
...
class DurableSession:
    ...
    def from_dict(payload: Mapping[str, object]) -> DurableSession:
        operator_context = _payload_mapping(
            payload.get("operator_context", {}), field_name="operator_context"
        )
        checkpoint_rows = _payload_sequence(
            payload.get("checkpoints", ()), field_name="checkpoints"
        )
        warnings = _payload_sequence(payload.get("warnings", ()), field_name="warnings")
        known_limitations = _payload_sequence(
            payload.get("known_limitations", ()), field_name="known_limitations"
...
class ReplayView:
    ...
    validation: SessionValidationResult = field(
        default_factory=lambda: SessionValidationResult(ok=True)
    )
    safety: SessionSafetyPosture = field(default_factory=SessionSafetyPosture)
