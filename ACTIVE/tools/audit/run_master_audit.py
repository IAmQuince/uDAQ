...
def main() -> int:
    ...
    parser.add_argument(
        "--profile", default="live-runtime-integration-and-safe-control-posture-foundations"
    )
    ...
    for row in contract_rows:
        if row["requirement_id"] not in valid_req:
            findings.append(
                f"execution contract references unknown requirement: {row['requirement_id']}"
            )
        missing_inv = sorted(set(_as_list(row.get("expected_invariant_ids", []))) - valid_inv)
        if missing_inv:
            findings.append(
                f"execution contract {row['requirement_id']} missing invariants: {missing_inv}"
            )
    ...
    for row in cov["rows"]:
        if row["implementation_entry_status"] == "READY_FOR_IMPLEMENTATION":
            if row["test_stub_status"] not in {"SCAFFOLDED", "IMPLEMENTED"}:
                findings.append(
                    f"ready requirement without scaffolded tests: {row['requirement_id']}"
                )
            if row["snapshot_status"] != "GENERATED":
                findings.append(
                    f"ready requirement without generated snapshot: {row['requirement_id']}"
                )
            if row["local_guardrail_status"] != "PRESENT":
                findings.append(
                    f"ready requirement without local guardrail: {row['requirement_id']}"
                )
    ...
    edge_rows = list(
        csv.DictReader(
            open(
                active_registry_path(root, "cross_reference_edges_csv"),
                newline="",
                encoding="utf-8",
            )
        )
    )
    edge_docs = {row["source_document_id"] for row in edge_rows} | {
        row["target_document_id"] for row in edge_rows
    }
    for row in doc_rows:
        ...
        if not path_value or not (root / path_value).exists():
            if active_state == "archive":
                continue
            findings.append(
                f"document registry path missing on disk: {doc_id or '<blank-id>'} -> {path_value}"
            )
            continue
        if not doc_id:
        ...
        if not revision:
            findings.append(
                f"document registry row missing revision: {doc_id or '<blank-id>'} -> {path_value}"
            )
        if not status:
            findings.append(
                f"document registry row missing status: {doc_id or '<blank-id>'} -> {path_value}"
            )
        if active_state == "active" and doc_id and doc_id not in edge_docs:
            findings.append(
                f"active document missing cross-reference graph participation: {doc_id}"
            )
