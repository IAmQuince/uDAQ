...
def launch_visible_operator_shell(*, initial_scenario_id: str = 'logic_control_demo', diagnostics_path: str | None = None, bench_mode: bool = False) -> int:
    ...
    class OperatorShellWindow(QtWidgets.QMainWindow):
        ...
        def _refresh_logic_watch(self, snapshot: Any) -> None:
            signal_map = {item.descriptor.signal_id: item for item in snapshot.signal_snapshots}
            if self.runtime_mode == 'LIVE':
                analog0 = signal_map.get('live::analog_in_0')
                analog1 = signal_map.get('live::analog_in_1')
                dac = signal_map.get('live::analog_out_0')
                lines = [
                    f'Runtime: LIVE ({snapshot.connection_state})',
                    'Bounded path: live readable inputs -> watch/graph -> guarded live write target',
                ]
                if analog0 is not None:
                    lines.append(f'Analog input 0: {analog0.value:0.3f} {analog0.descriptor.units or ""}'.strip())
                if analog1 is not None:
                    lines.append(f'Analog input 1: {analog1.value:0.3f} {analog1.descriptor.units or ""}'.strip())
                if dac is not None:
                    lines.append(f'Analog output 0 watch: {dac.value:0.3f} {dac.descriptor.units or ""}'.strip())
                self.logic_watch.setPlainText('\n'.join(lines))
                self.logic_inspector.clear()
                for label, item in (('live_analog_input_0', analog0), ('live_analog_input_1', analog1), ('guarded_analog_output_0', dac)):
                    if item is not None:
                        QtWidgets.QTreeWidgetItem(self.logic_inspector, [label, f'{item.value:0.3f} {item.descriptor.units or ""}'.strip()])
                self.logic_inspector.expandAll()
                return
            temp = signal_map.get('sig_demo_temp_0')
            flow = signal_map.get('sig_demo_flow_0')
            dac = signal_map.get('sig_demo_dac_cmd')
            lines = [
                f'Scenario: {snapshot.scenario.display_name}',
                'Pseudo path: edge input -> transform/filter -> pseudo analog sink',
            ]
            if temp is not None:
                lines.append(f'Temperature filtered: {temp.value:0.3f} {temp.descriptor.units}')
            if flow is not None:
                lines.append(f'Flow filtered: {flow.value:0.3f} {flow.descriptor.units}')
            if dac is not None:
                lines.append(f'Pseudo DAC output: {dac.value:0.3f} {dac.descriptor.units}')
            self.logic_watch.setPlainText('\n'.join(lines))
            self.logic_inspector.clear()
            if temp is not None:
                QtWidgets.QTreeWidgetItem(self.logic_inspector, ['temperature_source', f'{temp.value:0.3f} {temp.descriptor.units}'])
            if flow is not None:
                QtWidgets.QTreeWidgetItem(self.logic_inspector, ['flow_source', f'{flow.value:0.3f} {flow.descriptor.units}'])
            if dac is not None:
                QtWidgets.QTreeWidgetItem(self.logic_inspector, ['pseudo_dac_sink', f'{dac.value:0.3f} {dac.descriptor.units}'])
            self.logic_inspector.expandAll()
        ...
        def _build_logic_demo_scene(self) -> None:
            self.logic_scene.clear()
            boxes = [
                (20, 40, 180, 80, 'Pseudo Edge Input'),
                (280, 40, 220, 80, 'Filter / Mapping'),
                (560, 40, 220, 80, 'Pseudo Analog Output'),
            ]
            for x, y, w, h, label in boxes:
                rect = self.logic_scene.addRect(x, y, w, h, pen=QtGui.QPen(QtGui.QColor('#6ea8fe'), 2), brush=QtGui.QBrush(QtGui.QColor('#1f2933')))
                text = self.logic_scene.addText(label)
                text.setDefaultTextColor(QtGui.QColor('#f5f7fa'))
                text.setPos(x + 20, y + 24)
                _ = rect
            self.logic_scene.addLine(200, 80, 280, 80, pen=QtGui.QPen(QtGui.QColor('#7f8c8d'), 2, QtCore.Qt.DashLine))
            self.logic_scene.addLine(500, 80, 560, 80, pen=QtGui.QPen(QtGui.QColor('#7f8c8d'), 2, QtCore.Qt.DashLine))
