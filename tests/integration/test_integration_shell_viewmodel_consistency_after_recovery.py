...
TEST_DECLARATION = {
    ...
    'expected_proof_output': 'shell viewmodel remains consistent with review bundle after disconnect and recovery',
}
pytestmark = pytest.mark.integration


def test_shell_viewmodel_consistency_after_disconnect_and_recovery():
    controller, services, device = _bootstrap_controller(profile_id='PROF-VIEWMODEL-RECOVERY')
    ...
    assert view.device_phase == bundle['lifecycle_summary']['phase']
    assert view.lifecycle_summary is not None
    assert view.lifecycle_summary.published_signal_count == bundle['lifecycle_summary']['published_signal_count']
    assert view.binding_review_summary is not None
    assert view.binding_review_summary.total_signal_binding_count == bundle['binding_review_summary']['total_signal_binding_count']
    assert view.variable_health_summary is not None
    assert view.variable_health_summary.healthy_count == bundle['variable_health_summary']['healthy_count']
    assert view.workbench_review_state is not None
    assert view.workbench_review_state.total_workbench_count == bundle['workbench_review_state']['total_workbench_count']
