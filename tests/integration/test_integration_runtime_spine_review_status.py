...
TEST_DECLARATION = {
    ...
    'expected_proof_output': 'lifecycle review exposes bounded runtime spine status after repeated live polls through an optional support pack path',
}
...
def _bootstrap_controller(tmp_path):
    ...
    boot = ShellBootstrapper.bootstrap_from_profile(
        profile_snapshot=ProfileSnapshot(profile_id=ProfileId('PROF-RUNTIME-SPINE'), workspace_state=WorkspaceState(page='graphing', review_mode=GraphMode.LIVE)),
...
def test_runtime_spine_status_shows_bounded_recent_state(tmp_path):
    controller, services = _bootstrap_controller(tmp_path)
    controller.poll_adapters(timestamp=as_event_time(8))
    controller.poll_adapters(timestamp=as_event_time(9))

    bundle = controller.lifecycle_review_bundle()
    status = bundle['runtime_status']
    ...
    assert services.runtime_metrics.snapshot()['timings']['runtime.acquisition.poll.ms']['count'] >= 3
