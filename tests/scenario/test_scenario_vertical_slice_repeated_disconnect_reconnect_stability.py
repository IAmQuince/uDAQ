...
TEST_DECLARATION = {
    'test_id': 'UDQ-TST-SCN-015',
    'verifies_requirements': ['UDQ-REQ-ARCH-001', 'UDQ-REQ-UI-006'],
    'checks_invariants': ['UDQ-INV-STATE-001'],
    'worked_example_reference': None,
    'expected_proof_output': 'repeated disconnect/reconnect cycles do not duplicate projected points or inflate review summaries',
}
pytestmark = pytest.mark.scenario


def test_vertical_slice_repeated_disconnect_reconnect_stability():
    controller, services, device = _bootstrap_controller(profile_id='PROF-REPEAT-CHURN')
    ...
    initial_point_count = len(services.bindings.point_definitions)

    for offset in range(3):
        controller.mark_active_device_disconnected(timestamp=as_event_time(8 + offset * 2))
        controller.reconnect_active_device(timestamp=as_event_time(9 + offset * 2))

    bundle = controller.lifecycle_review_bundle()
    assert len(services.bindings.point_definitions) == initial_point_count
    ...
    assert len(bundle['transition_trace']) >= 6
