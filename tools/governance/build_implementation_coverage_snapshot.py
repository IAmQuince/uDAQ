...
def main() -> int:
    ...
    parser.add_argument('--output', default='audit_reports/active/UDQ_IMPLEMENTATION_COVERAGE_SNAPSHOT__2026-03-21_235959.json')
    args = parser.parse_args()
    root = Path(args.package_root).resolve()
    coverage = load_json(active_registry_path(root, 'implementation_coverage_json'))
    payload = {'generated_at': timestamp_slug(), 'rows': coverage['rows']}
