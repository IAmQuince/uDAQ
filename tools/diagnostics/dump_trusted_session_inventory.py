...
def build_controller(*, disconnect_after_polls: int | None = None, stale_after_polls: int | None = None) -> ShellController:
    ...
    boot = ShellBootstrapper.bootstrap_from_profile(
        profile_snapshot=ProfileSnapshot(
            profile_id=ProfileId('PROF-TRUSTED-SESSION'),
        ...
        actor_context=ActorContext(actor_id=ActorId('trusted-session-operator'), role_class=RoleClass.OPERATOR, origin='local-shell'),
        services=services,
    )
    return ShellController.from_bootstrapped_shell(boot)


def build_inventory(*, sample_count: int, reconnect: bool, disconnect_after_polls: int | None = None, stale_after_polls: int | None = None) -> dict[str, object]:
    ...
    first_plotted_point_at: int | None = None
    for offset in range(sample_count):
        controller.poll_adapters(timestamp=as_event_time(6 + offset))
        summary = controller.session.ui_session.first_signal_summary
        if first_plotted_point_at is None and summary is not None and summary.trace_point_count >= 2 and summary.latest_timestamp is not None:
            first_plotted_point_at = int(summary.latest_timestamp)

    if reconnect:
        controller.mark_active_device_disconnected(timestamp=as_event_time(50))
        start = time.perf_counter()
        controller.reconnect_active_device(timestamp=as_event_time(51))
        timings['reconnect_ms'] = round((time.perf_counter() - start) * 1000.0, 3)
        controller.poll_adapters(timestamp=as_event_time(52))
        controller.poll_adapters(timestamp=as_event_time(53))

    view = controller.view_model()
    replay = FirstSignalReplayTape.from_summary(controller.session.ui_session.first_signal_summary)
    trusted = controller.trusted_session_inventory()
    return {
        ...
        'timings_ms': timings,
        'first_plotted_point_at': first_plotted_point_at,
        ...
        'trusted_session_summary': trusted,
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Emit a deterministic trusted-session diagnostic inventory.')
