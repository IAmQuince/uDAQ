...
def main() -> int:
    ...
    boot = ShellBootstrapper.bootstrap_from_profile(
        profile_snapshot=ProfileSnapshot(profile_id=ProfileId("PROF-PERF"), workspace_state=WorkspaceState(page="graphing", review_mode=GraphMode.LIVE)),
        ...
        actor_context=ActorContext(actor_id=ActorId("perf-check"), role_class=RoleClass.ENGINEER, origin="local-shell"),
        services=services,
    )
    ...
    payload = {
        ...
        "object_counts": {
            "adapter_count": len(services.adapters.adapters),
            "discovered_device_count": len(services.adapters.discovered_devices),
            "device_record_count": len(services.device_registry.device_records),
            "signal_count": len(services.signals.definitions),
            "variable_count": len(services.variables.definitions),
