...
def main() -> int:
    parser = argparse.ArgumentParser(description='Run a bounded UniversalDAQ LabJack U6 command-admission smoke test.')
    parser.add_argument('--package-root', default='.')
    parser.add_argument('--summary', default='proof/U6_COMMAND_SUMMARY.txt')
    parser.add_argument('--journal', default='proof/U6_COMMAND_JOURNAL.jsonl')
    ...
    try:
        ...
        install_labjack_support_pack(
            ...
            simulated_serial_number='470177',
        )
        controller = bootstrap_controller(services=services, profile_id='PROF-U6-CMD', actor_id='u6-command')
        ...
        run_poll_cycles(controller, timestamp_start=1005, cycles=1)
        ...
        lines.extend(['UDQ U6 Command Smoke', '====================', json.dumps(summary_payload, indent=2, sort_keys=True)])
    ...
    print(f'udq-u6-command-smoke: wrote {summary_path}')
    return exit_code
