...
def main() -> int:
    parser = argparse.ArgumentParser(description='Run a bounded UniversalDAQ LabJack U6 events/alarms smoke test.')
    parser.add_argument('--package-root', default='.')
    parser.add_argument('--summary', default='proof/U6_EVENT_ALARM_SUMMARY.txt')
    parser.add_argument('--journal', default='proof/U6_EVENT_ALARM_JOURNAL.jsonl')
    ...
    try:
        ...
        controller = bootstrap_controller(services=services, profile_id='PROF-U6-EVT', actor_id='u6-event')
        ...
        run_poll_cycles(controller, timestamp_start=1003, cycles=2)
        ...
        lines.extend(['UDQ U6 Event Alarm Smoke', '========================', json.dumps(summary_payload, indent=2, sort_keys=True)])
    ...
    print(f'udq-u6-event-alarm-smoke: wrote {summary_path}')
    return exit_code
